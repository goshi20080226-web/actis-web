...
export default function DepartureBoard(){
 ...
 const upcoming=useMemo(()=>trains.filter(t=>dir==="ALL"||t.direction===dir).map(t=>{const s=stationOf(t,station);if(!s||s.isPass||String(s.stopType??"1")==="2")return null;const raw=s.departure||s.single,n=sec(raw);if(n<0)return null;let actual=n;while(actual-nowSec<-5)actual+=86400;return{id:t.id||t.trainNo,t,type:t.type||t.typeShort||"普通",time:fmt(raw),actual,diff:actual-nowSec,dest:destination(t),track:s.track||s.trackName||"",cars:t.cars||t.carCount?"":"" ,msg:nextMessage(t,station),color:typeColor(t,led==="3color")}}).filter(Boolean).sort((a,b)=>a.actual-b.actual),[trains,dir,station,nowSec,led])
